...
export async function modificarPrediccion(idPartido, userId, resultadoEquipo1, resultadoEquipo2) {
  const instance = axios.create({
    baseURL: 'http://127.0.0.1:8080',
    headers: {
      'Access-Control-Allow-Origin': '*',
  ...
  let res = instance.put("http://127.0.0.1:8080/prediccion/" + idPartido + "/modificarPrediccion?userId=" + userId + "&resultadoEquipo1=" + resultadoEquipo1 + "&resultadoEquipo2=" + resultadoEquipo2)
    .then(response => {
      let serviceResponse = [];
      serviceResponse[0] = response.data.defaultResponse;
      serviceResponse[1] = response.data.partidos;
