...
import { getCarreras, createUser, createAlumno, login } from "../Services/UserService"
...
const LoginComponent = (props) => {
  ...
  const createUserInDatabase = () => {
    createUser(username, password).then(createUserResponse => {
      if (createUserResponse[1].response.code == '200') {
        createAlumno(createUserResponse[1].jwt, cedulaIdentidad, name, suername, bithDate, mail, carrera, username).then((data) => {
  ...
  return (
    <>
            ...
            <div>
              <div>
                ...
                <form class="background" >
                  ...
                  <p className="Login-Component-RegisterText" onClick={changeMode}>Register</p>
                </form>
              </div>
            </div>
            :
              ...
              <form class="Register-Component-Form" >
                ...
                <select id="carrera" name="Carrera" onChange={(event) => setCarrera(event.target.value)}>
                  {carreras.map((carrera, i) => {
                    return (
                      <option value={carrera.idCarrera} key={i}>{carrera.nombreCarrera}</option>
                    );
                  })}
                ...
                <button
                  className="Login-Component-Button"
                  type="button"
                  onClick={createUserInDatabase}>Register</button>
                <p className="Login-Component-Text">Back to</p>
                <p className="Login-Component-RegisterText" onClick={changeMode}>Login</p>
              </form>
            </div>
        }
      </div>
      ...
      <Alert className="Login-Component-Alert" key={"loginFailed"} variant={"danger"} show={failedLogin} dismissible onClose={() => setFailedLogin(false)}>
        Error during login process
      </Alert>

      <Alert className="Login-Component-Alert" key={"registerFailed"} variant={"danger"} show={failedRegister} dismissible onClose={() => setFailedRegister(false)}>
